...
def recorre_lib(detalles_coincidencias,detalles_coincidencia_copiaas):
    for num_linea, detalle in detalles_coincidencias.items():
        cl_actual = detalle['Cl_especifico']
        codigo_binario_completo = ''.join(detalle['Codigo_binario'])  # Concatena todos los códigos binarios
        
        cl_actual=str(cl_actual).zfill(4)
        cl_actual=decimal_a_hexadecimal(cl_actual)
        # Accede a la información de la línea desde el detalle actual
        info_linea = detalle.get('linea_contenido', 'N/A')
        # Imprime la información con el formato adecuado
        print(f"{cl_actual}              {codigo_binario_completo}   {info_linea} ")
...
if __name__ == "__main__":
    ...
    bol=comparar(archivo_asm, expresiones_regulares)  
    if bol==True :
        print("-------------------------------------")
        for etiqueta, info in etiquetas_info.items():
        ...
        print("------------------------- Segunda pasada completa -------------")        
        recorre_lib(detalles_coincidencias,detalles_coincidencias_copia)
